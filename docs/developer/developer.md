...
网页使用iframe加载游戏播放器的`html`文件，并通过传递消息的方式与游戏播放器进行必要的通讯。

### 2. 与评测机对接

用户在网站上以人类玩家的身份加入一个房间后，即可接入一个对局，并由人类进行游戏操作。在这之中，播放器与评测机会有如下的远程通信：
...
```javascript
token = JUDGER_BASE_URL + "/" + ROOM_ID + "/" + USER_NAME + "/" + SEAT
```

其中，`JUDGER_BASE_URL`为评测机服务器的地址，`ROOM_ID`为房间编号，`USER_NAME`为播放器用户的用户名，`SEAT`为该播放器对应在房间中的座位号。

根据该`token`，播放器应作为客户端与`wss://token`建立`websocket`连接，例如：`wss://ares.judge.saiblo.net/human/8/Aglove/1`，这表明一个用户名为`Aglove`的用户在房间`8`的位置`1`。
...
游戏部署方可以选择是否为播放器提供计时接口。**即使播放器不需要这一接口，我们也仍然建议游戏部署方勾选这一选项。**
...
其中`remain_time`表示评测机认为该玩家距离该回合超时的剩余时间。
