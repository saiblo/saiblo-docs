...
当游戏结束、逻辑完成其他收尾工作后，逻辑需向 judger 发送游戏结束消息，并告知各玩家的分数。示例如下：
...
```javascript
token = JUDGER_BASE_URL + "/" + ROOM_ID     // 房间
token = JUDGER_BASE_URL + "/_" + MATCH_ID   // 对局
```

其中，`JUDGER_BASE_URL`为评测机服务器的地址，`ROOM_ID`为房间编号，`MATCH_ID`为对局编号。

根据该`token`，播放器应作为客户端与`wss://token`建立`websocket`连接，例如：`wss://ares.judge.saiblo.net/human/8`，这表明加入房间`8`的观战。
